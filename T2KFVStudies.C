...
#include "/nfs/data41/t2k/amissert/roottools/GetCumulative.C"
...
void T2KFVStudies::drawPassFail(const char* var,int isamp){
  ...
  //
  canvas->cd(1);
  histosCompare[0]->stack[isamp]->Draw("h");
  //
  canvas->cd(2);
  histosCompare[1]->stack[isamp]->Draw("h");
  //
  canvas->cd(3);
  histosCompare[2]->stack[isamp]->Draw("h");
  //
  canvas->cd(4);
  histosCompare[3]->stack[isamp]->Draw("h");
...
void T2KFVStudies::fill(FVStudyHistos* hh){
    ...
    if (passedECuts){
      ...
      if (TMath::Abs(mcEvent->mode)==1){
        ...
        hh->heresvwall[0]->Fill(mcEvent->fqtowall,mcEvent->fqwall,NormPOT*mcEvent->evtweight);
      } //< end CCQE block
      ...
      hh->hpos[icat][isamp]->Fill(TMath::Sqrt(mcEvent->posv[0]*mcEvent->posv[0] + mcEvent->posv[1]*mcEvent->posv[1]),TMath::Abs(mcEvent->posv[2]),NormPOT*mcEvent->evtweight); 
    ...
    if (passedMuCuts){
      ...
      float momres = (fqpars.fqenumu - mcEvent->pmomv[0] );
        ...
        hh->heresvwall[1]->Fill(mcEvent->fqtowall,mcEvent->fqwall,NormPOT*mcEvent->evtweight);
      } //< end CCQE block
...
/*    // get catagory//////////////////////////////
    ...
    if (mcEvent->passecut){
      icat = getCatagory(12);
      isamp = 0;
      histos[ivol]->herec[icat][isamp]->Fill(fqpars.fqenue, NormPOT*mcEvent->evtweight);
    }
    //
    if (mcEvent->passmucut){
      icat = getCatagory(14);
      isamp = 1;
      histos[ivol]->herec[icat][isamp]->Fill(fqpars.fqenumu,NormPOT*mcEvent->evtweight);
    }

    // fv stuff
    histos[ivol]->hwall[icat][isamp]->Fill(mcEvent->fqwall, NormPOT*mcEvent->evtweight);
    histos[ivol]->hwallv[icat][isamp]->Fill(mcEvent->wallv, NormPOT*mcEvent->evtweight);
    histos[ivol]->hetrue[icat][isamp]->Fill(mcEvent->pmomv[0], NormPOT*mcEvent->evtweight);
    histos[ivol]->htowall[icat][isamp]->Fill(mcEvent->fqtowall, NormPOT*mcEvent->evtweight);
    histos[ivol]->htowallv[icat][isamp]->Fill(mcEvent->towallv[2], NormPOT*mcEvent->evtweight);
    histos[ivol]->hfv[icat][isamp]->Fill(mcEvent->fqtowall,mcEvent->fqwall,NormPOT*mcEvent->evtweight);
*/
   //
   return;
}
