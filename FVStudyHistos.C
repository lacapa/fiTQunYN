...
//       hfv[icat][isample]->Reset();
       // clear the count too
       eventCount[icat][isample];
    }
    heres[isample]->Reset();
...
void FVStudyHistos::init(){
  ...
  for (int icat=0; icat<NCAT; icat++){
    for (int isamp=0; isamp<NSAMP; isamp++){
      ...
      hfv[icat][isamp] = new TH2FV(hname.Data(),-1,30,0,300,30,0,300);
      hfv[icat][isamp]->SetName(hname.Data());
      // position
      hname = nameTag.Data();
      hname.Append(Form("hposition_%d",isamp));
      hpos[icat][isamp] = new TH2D(hname.Data(),hname.Data(),200,0,1800,200,0,1800);
  ...
  hfvtot[0] = new TH2FV(hname.Data(),-1,30,0,300,30,0,300);
  hfvtot[0]->SetName(hname.Data());
  hname = nameTag.Data();
  hname.Append("hfvtot_numu");
  hfvtot[1] = new TH2FV(hname.Data(),-1,30,0,300,30,0,300);
  hfvtot[1]->SetName(hname.Data());
  //
  hname = nameTag.Data();
  hname.Append("hfvtottrue_nue");
  hfvtottrue[0] = new TH2FV(hname.Data(),-1,30,0,300,30,0,300);
  hfvtottrue[0]->SetName(hname.Data());
  hname = nameTag.Data();
  hname.Append("hfvtottrue_numu");
  hfvtottrue[1] = new TH2FV(hname.Data(),-1,30,0,300,30,0,300);
  ...
  // erec
  double maxE[2] = {1200.,3000.};
  TH1D* erecseed[2];
  erecseed[0] = new TH1D("erec","erec",nbins,0,maxE[0]);
  erecseed[1] = new TH1D("erec","erec",nbins,0,maxE[1]);
  ...
  for (int isamp=0; isamp<NSAMP; isamp++){
    ...
    heres[isamp]  = new TH1D(hname.Data(),hname.Data(),200,-1000,1000);
    hname = nameTag.Data();
    hname.Append(Form("_hdirresidual_samp%d",isamp));
    hdirres[isamp]  = new TH1D(hname.Data(),hname.Data(),200,-1,1);
    hname = nameTag.Data();
